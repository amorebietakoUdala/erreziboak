...
{
    final public const SOURCE_MIPAGO = 0;
    final public const SOURCE_OTHER = 1;
    ...
    {
        $payment_response = json_decode((string) $json, true, 512, JSON_THROW_ON_ERROR);
        $payment = new self();
        $payment->setSource(self::SOURCE_OTHER);
        $payment->setReferenceNumber(str_pad((string) $payment_response['reference_number'], 10, '0', STR_PAD_LEFT));
        ...
        $payment->setPaymentDate(str_replace('/', '', (string) $payment_response['Ds_Date']));
        $payment->setPaymentHour(str_replace(':', '', (string) $payment_response['Ds_Hour']));
